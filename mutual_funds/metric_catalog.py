...
METRIC_GROUPS: dict[str, tuple[str, ...]] = {
    ...
    "Returns": (
        "3M Return %", "6M Return %", "1Y Abs Return %",
        "1Y CAGR %", "3Y CAGR %", "5Y CAGR %", "10Y CAGR %",
        "1Y Cum Return %", "Avg Daily Return %",
    ),
    "Rolling returns": (
        "Roll 1Y Min %", "Roll 1Y Median %", "Roll 1Y Mean %", "Roll 1Y Max %",
        "Roll 3Y Min %", "Roll 3Y Median %", "Roll 3Y Mean %", "Roll 3Y Max %",
        "Roll 5Y Min %", "Roll 5Y Median %", "Roll 5Y Mean %", "Roll 5Y Max %",
    ),
    ...
    "Distribution stats": (
        ...
        "VaR 95% (Daily)", "CVaR 95% (Daily)", "Skew", "Kurtosis",
    ),
...
METRIC_PCT_COLS: tuple[str, ...] = (
    "1Y CAGR %", "3Y CAGR %", "5Y CAGR %", "10Y CAGR %",
    "1Y Cum Return %", "Avg Daily Return %",
    "1Y Vol %", "1Y Downside Vol %",
    "Max DD 1Y %", "Max DD All %", "% from ATH",
    "Win Rate %", "Best Day %", "Worst Day %",
    "VaR 95% (Daily)", "CVaR 95% (Daily)",
    "Avg Win %", "Avg Loss %",
    "3M Return %", "6M Return %", "1Y Abs Return %",
    "Alpha (1Y) %", "Tracking Error %",
    "Roll 1Y Min %", "Roll 1Y Median %", "Roll 1Y Mean %", "Roll 1Y Max %",
    "Roll 3Y Min %", "Roll 3Y Median %", "Roll 3Y Mean %", "Roll 3Y Max %",
    "Roll 5Y Min %", "Roll 5Y Median %", "Roll 5Y Mean %", "Roll 5Y Max %",
    "% Equity", "% Debt", "% Cash",
...
METRIC_NUMERIC_COLS: frozenset[str] = frozenset(
    {*METRIC_PCT_COLS, "AUM (₹ Cr)", "TER %", "Sharpe", "Sortino", "Calmar", "Gain/Pain",
     "Skew", "Kurtosis", "Kelly %", "Payoff", "Beta (1Y)", "R² (1Y)"}
)
...
DISPLAY_COL_ORDER: tuple[str, ...] = IDENTITY_COLS + tuple(
    c for cols in METRIC_GROUPS.values() for c in cols
)
...
EMPTY_METRIC_DB_COLS: tuple[str, ...] = (
    ...
    "win_rate_1y", "best_day_1y", "worst_day_1y", "var_95_1y", "cvar_95_1y",
    "skew_1y", "kurt_1y",

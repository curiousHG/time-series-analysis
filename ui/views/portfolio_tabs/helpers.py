"""Shared helpers for portfolio tabs."""

import polars as pl
import pandas as pd

from data.store.mutual_fund import ensure_fund_mapping, _load_nav_df
...
def get_mapped_data(txn_df: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame] | None:
    ...
    portfolio_nav = _load_nav_df(all_schemes)

    return mapped, portfolio_nav


def build_portfolio_value_series(
    mapped: pl.DataFrame, nav_df: pl.DataFrame
) -> pd.DataFrame | None:

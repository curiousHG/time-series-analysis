...
def _render_summary_metrics(returns: pd.Series):
    ...
    c1.metric(
        "CAGR",
        f"{qs.stats.cagr(returns) * 100:.2f}%",
    ...
    c19.metric("Avg Loss", f"{avg_l:.3f}%", help="Average return on losing days. Smaller magnitude is better.")
    c20.metric(
        "Payoff Ratio",
        f"{payoff:.2f}",
...
def _render_charts(returns: pd.Series, pv: pd.DataFrame):
    ...
    with col1:
        ...
        fig_hist.update_layout(
            height=350,
            title="Daily Returns Distribution (Cash-Flow Adjusted)",
            xaxis_title="Return (%)",
            yaxis_title="Count",
        )
    ...
    # Rolling Sharpe
    rolling_sharpe = (returns.rolling(60).mean() * 252 - RISK_FREE) / (returns.rolling(60).std() * np.sqrt(252))
